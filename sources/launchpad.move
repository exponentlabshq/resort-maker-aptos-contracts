module launchpad_addr::launchpad {
    ...
    /// Mint tokens to a specific address (for staking rewards)
    /// Only callable by the launchpad module itself or admin
    public fun mint_to(
        fa_obj: Object<Metadata>,
        recipient: address,
        amount: u64
        ...
        let fa_obj_addr = object::object_address(&fa_obj);
        let fa_controller = borrow_global<FAController>(fa_obj_addr);
        primary_fungible_store::mint(&fa_controller.mint_ref, recipient, amount);
    }
